...
public static class MigrationPlanMerger
{
            ...
            foreach (var entry in plan.Entries)
            {
                ...
                if (!string.Equals(existing.Value, effectiveEntry.Value, StringComparison.Ordinal))
                {
                    if (renameConflictingKeys)
                    {
                        ...
                        warnings.Add(
                            $"Conflicting Source key '{effectiveEntry.Key}' in env '{effectiveEntry.Environment}' has different values. " +
                    ...
                    warnings.Add(
                        $"Skipped conflicting Source entry for key '{effectiveEntry.Key}' in env '{effectiveEntry.Environment}'. " +
