...
describe('UsersPage', () => {
  ...
  it('does not offer password reset for passwordless users or editors', async () => {
    server.use(
    ...
    firstRender.unmount();
    localStorage.setItem('auth_session', JSON.stringify({ email: mockUsers[0].email, role: 'editor' }));
    renderWithProviders(() => <UsersPage />);
    await screen.findByText(mockUsers[0].email);
    expect(screen.queryByTestId(`team-password-reset-${mockUsers[0].id}`)).not.toBeInTheDocument();
  });
