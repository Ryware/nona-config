...
public static class HttpReadBenchmarkApp
{
    ...
    {
        return $"/api/environments/{GetEnvironmentName(datasetKeyCount)}/parameters";
    }
    ...
    private static string BuildMarkdown(HttpReadSummary summary)
    {
        ...
        {
            builder.AppendLine("- Full-environment reads use `GET /api/environments/{environment}/parameters`.");
        }
        if (summary.Results.Any(result => result.Operation == HttpReadOperation.SingleKey))
        {
            builder.AppendLine("- Single-key reads use `GET /api/environments/{environment}/parameters/{key}` for one fixed key in the 10,000-key dataset at concurrency 1, 50, and 100.");
        }
