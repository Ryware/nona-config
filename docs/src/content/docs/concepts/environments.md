...
- release routes without `version` read the active release, or return `409` when none is active
- release routes with `version=1.1.0` read that exact release
- release routes with `version=1.1.x` read the highest patch in the `1.1` line
...
When no release is active, unversioned release reads fail with `active_release_not_configured`; they never switch to working parameters.
